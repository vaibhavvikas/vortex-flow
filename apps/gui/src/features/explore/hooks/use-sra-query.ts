...
export function useSraQuery(query: string, db: string, page: number) {
  return useQuery<SraSearchResponse>({
    ...
    placeholderData: (previousData: SraSearchResponse | undefined) => previousData, // Keeps previous page data visible while fetching next page for smooth transitions
  })
}
