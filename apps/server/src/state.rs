      ...
      while let Ok(event) = rx.recv().await {
        match event {
          ...
          DownloadEvent::TaskFinished {
            ...
            if destination.extension().and_then(|e| e.to_str()) == Some("zip") {
              ...
              tokio::task::spawn_blocking(move || {
                match vortexflow_storage::extract_genome_package(&dest_clone, &genomes_dir) {
                  Ok(extracted_genomes) => {
                    for genome in extracted_genomes {
                      ...
                      let _ = service_clone.mark_item_downloaded(&genome.accession);
                    }
          ...
          DownloadEvent::ProgressUpdated(p) => {
            ...
            if p.downloaded_bytes > 0 && p.total_bytes > 0 && p.downloaded_bytes == p.total_bytes {
              let _ = service_for_events.update_file_download_progress(
                &p.task_id,
                "completed",
