...
pub enum SocketType {
    ...
    FastqPair,
    /// Output directory generated by ResFinder
    #[serde(rename = "resfinder_output_folder", alias = "resfinder_report", alias = "res_finder_report")]
    ResFinderOutputDir,
    /// Parsed AMR gene resistance table
    #[serde(rename = "amr_gene_table", alias = "table:amr_genes")]
    AmrGeneTable,
    /// Parsed PointFinder chromosomal mutations table
    #[serde(rename = "point_mutation_table", alias = "table:point_mutations")]
    PointMutationTable,
    /// Predicted antimicrobial phenotype profile
    #[serde(rename = "phenotype_table", alias = "table:phenotypes")]
    PhenotypeTable,
    /// Raw TSV / Tabular data file
    #[serde(rename = "tsv_file", alias = "file:tsv")]
    TsvFile,
    /// Rendered HTML report (e.g. FastQC)
    #[serde(rename = "html_report", alias = "report:html")]
    HtmlReport,
    /// Generic Tabular/TSV data
    TabularReport,
    /// Structured JSON data payload
    #[serde(rename = "json_data", alias = "data:json")]
    JsonData,
...
impl SocketType {
    ...
    pub fn can_connect_to(&self, target: &SocketType) -> bool {
        ...
        if *self == SocketType::TsvFile && *target == SocketType::TabularReport {
            return true;
        }
        if *self == SocketType::TabularReport && *target == SocketType::TsvFile {
            return true;
        }
        self == target
    }
    ...
    pub fn template_resfinder() -> Self {
        ...
        let node_resfinder = WorkflowNode {
            ...
            outputs: vec![Port {
                ...
                socket_type: SocketType::ResFinderOutputDir,
                direction: PortDirection::Output,
            }],
            params: serde_json::json!({
                "threshold": 0.90,
